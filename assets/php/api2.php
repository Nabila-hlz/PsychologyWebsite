...
function addVideo($conn) {
    ...
    $embed_path = "https://www.youtube.com/embed/" . $youtube_id;

    $stmt = $conn->prepare("INSERT INTO content (TYPE, PATH, TITLE, DISCRIPTION, THERAPIST_ID, SPECIALTY_ID, VIEWS, CREATED_AT) VALUES ('Video', ?, ?, ?, ?, ?, 0, NOW())");
    $stmt->bind_param("sssii", $embed_path, $title, $description, $therapist_id, $specialty_id);
...
function deleteVideo($conn) {
    ...
    if ($stmt->execute() && $stmt->affected_rows > 0) {
        ...
        echo json_encode(['success' => false, 'message' => 'Video not found or could not be deleted']);
    }
